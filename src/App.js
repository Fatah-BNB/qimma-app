...
import './App.css'
import Profile from "./components/user/profile/settings";
import Profile1 from "./components/user/profile/profile";
import CoursePayment from "./components/user/home/course-details/course-payment";
import AdminManagment from "./components/admin/admin-managment";
import StudyCourse from "./components/student/course-library/study-course";
import CourseResources from "./components/instructor/course-resources/course-resources";
    ...
    <Router>
      <div className="App">
        <Routes>
          ...
          <Route path="/profile" element={<ProtectedRoute isLogged={isLogged} child={<Profile1/>} redirect="/login" />} />
          <Route path="/settings" element={<ProtectedRoute isLogged={isLogged} child={<Profile/>} redirect="/login" />} />
          ...
          <Route path="/admin-management" element={<ProtectedRoute isLogged={adminIsLogged} child={<AdminManagment />} redirect="/login-admin" />} />\
          ...
          <Route path="/instructor-course-resources" element={<ProtectedRoute isLogged={isLogged} child={<CourseResources />} redirect="/login" />} />\
          
          <Route path="/course-library" element={<ProtectedRoute isLogged={isLogged} child={<CourseLibrary />} redirect="/login" />} />\
          <Route path="/study-course" element={<ProtectedRoute isLogged={isLogged} child={<StudyCourse />} redirect="/login" />} />\

          <Route path="/course-details" element={<ProtectedRoute isLogged={isLogged} child={<CourseDetails />} redirect="/login" />} />\
          <Route path="/course-payment" element={<ProtectedRoute isLogged={isLogged} child={<CoursePayment />} redirect="/login" />} />\
